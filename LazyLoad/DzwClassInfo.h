...
@interface DzwClassInfo : NSObject
...
/**
 *  当前类里面属性对应为类的字典 [key->JSON字段的key : value->DzwClassInfo对象]
 */
@property (nonatomic, strong) NSMutableDictionary *propertyClassDic;

/**
 *  当前类里属性对应为集合(指定集合里面存某个模型，用作MJExtension) [key->JSON字段的key : value->DzwClassInfo对象]
 */
